...
export class BrowserPool {
  ...
  constructor() {
    ...
    console.warn(`[BrowserPool] Configuration: maxBrowsers=${this.maxBrowsers}, headless=${this.headless}, types=${this.browserTypes.join(',')}`);
  }
  ...
  async getBrowser(): Promise<Browser> {
    ...
    if (this.browsers.has(browserType)) {
      ...
      try {
        ...
        console.warn(`[BrowserPool] Browser ${browserType} health check failed:`, error);
        // Browser is unhealthy, remove it and close if possible
        this.browsers.delete(browserType);
        try {
          await browser.close();
        } catch (closeError) {
          console.warn(`[BrowserPool] Error closing unhealthy browser:`, closeError);
        }
      }
    }

    // Launch new browser
    console.warn(`[BrowserPool] Launching new ${browserType} browser`);
  ...
  async closeAll(): Promise<void> {
    console.warn(`[BrowserPool] Closing ${this.browsers.size} browsers`);
