...
export class EnhancedContentExtractor {
  ...
  constructor() {
    ...
    console.warn(`[EnhancedContentExtractor] Configuration: timeout=${this.defaultTimeout}, maxContentLength=${this.maxContentLength}, fallbackThreshold=${this.fallbackThreshold}`);
  }

  async extractContent(options: ContentExtractionOptions): Promise<string> {
    const { url } = options;
    
    console.warn(`[EnhancedContentExtractor] Starting extraction for: ${url}`);
    
    // First, try with regular HTTP client (faster)
    try {
      const content = await this.extractWithAxios(options);
      console.warn(`[EnhancedContentExtractor] Successfully extracted with axios: ${content.length} chars`);
      return content;
    } catch (error) {
      ...
      if (this.shouldUseBrowser(error, url)) {
        console.warn(`[EnhancedContentExtractor] Falling back to headless browser for: ${url}`);
        try {
          const content = await this.extractWithBrowser(options);
          console.warn(`[EnhancedContentExtractor] Successfully extracted with browser: ${content.length} chars`);
          return content;
        } catch (browserError) {
  ...
  private async extractWithAxios(options: ContentExtractionOptions): Promise<string> {
    ...
    if (maxContentLength && content.length > maxContentLength) {
      console.warn(`[EnhancedContentExtractor] Content truncated from ${content.length} to ${maxContentLength} characters for ${url}`);
  ...
  private async extractWithBrowser(options: ContentExtractionOptions): Promise<string> {
    ...
    try {
      ...
      console.warn(`[BrowserExtractor] Navigating to ${url}`);
      ...
      try {
        ...
        if (errorMessage.includes('ERR_HTTP2_PROTOCOL_ERROR') || errorMessage.includes('HTTP2')) {
          console.warn(`[BrowserExtractor] HTTP/2 error detected, trying with HTTP/1.1`);
      ...
      try {
        ...
        console.warn(`[BrowserExtractor] No main content selector found, proceeding anyway`);
      }
  ...
  private async simulateHumanBehavior(page: Page): Promise<void> {
    try {
      ...
      console.warn(`[BrowserExtractor] Behavior simulation failed, continuing`);
    }
  }
  ...
  async extractContentForResults(results: SearchResult[], targetCount: number = results.length): Promise<SearchResult[]> {
    console.warn(`[EnhancedContentExtractor] Processing up to ${results.length} results to get ${targetCount} non-PDF results`);
    ...
    console.warn(`[EnhancedContentExtractor] Processing ${resultsToProcess.length} non-PDF results concurrently`);
    ...
    const extractionPromises = resultsToProcess.map(async (result): Promise<SearchResult> => {
      try {
        ...
        console.warn(`[EnhancedContentExtractor] Successfully extracted: ${result.url}`);
        return {
          ...result,
        ...
        console.warn(`[EnhancedContentExtractor] Failed to extract: ${result.url} - ${error instanceof Error ? error.message : 'Unknown error'}`);
        return {
          ...result,
          fullContent: '',
          contentPreview: '',
    ...
    console.warn(`[EnhancedContentExtractor] Completed processing ${resultsToProcess.length} results, extracted ${successfulResults.length} successful/${failedResults.length} failed`);
  ...
  private parseContent(html: string): string {
    ...
    for (const selector of contentSelectors) {
      ...
      if ($content.length > 0) {
        ...
        if (mainContent.length > 100) { // Ensure we have substantial content
          console.warn(`[EnhancedContentExtractor] Found content with selector: ${selector} (${mainContent.length} chars)`);
          break;
        }
      }
    }
    ...
    if (!mainContent || mainContent.length < 100) {
      console.warn(`[EnhancedContentExtractor] No main content found, using body content`);
