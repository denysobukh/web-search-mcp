#!/usr/bin/env node
console.warn('Web Search MCP Server starting...');
...
class WebSearchMCPServer {
  ...
  private setupTools(): void {
    ...
    this.server.tool(
      ...
      async (args: unknown) => {
        console.warn(`[MCP] Tool call received: full-web-search`);
        console.warn(`[MCP] Raw arguments:`, JSON.stringify(args, null, 2));
        ...
        try {
          ...
          if (!hasExplicitMaxLength && isLikelyLlama) {
            console.warn(`[MCP] Detected potential Llama model (string parameters), applying content length limit`);
          ...
          if (isLikelyRobustModel && validatedArgs.maxContentLength && validatedArgs.maxContentLength < 5000) {
            console.warn(`[MCP] Detected robust model (numeric parameters), removing unnecessary content length limit`);
            validatedArgs.maxContentLength = undefined;
          }
          
          console.warn(`[MCP] Validated args:`, JSON.stringify(validatedArgs, null, 2));
          
          console.warn(`[MCP] Starting web search...`);
          const result = await this.handleWebSearch(validatedArgs);
          
          console.warn(`[MCP] Search completed, found ${result.results.length} results`);
    ...
    this.server.tool(
      ...
      async (args: unknown) => {
        console.warn(`[MCP] Tool call received: get-web-search-summaries`);
        console.warn(`[MCP] Raw arguments:`, JSON.stringify(args, null, 2));
        ...
        try {
          ...
          console.warn(`[MCP] Starting web search summaries...`);
          ...
          try {
            ...
            console.warn(`[MCP] Search summaries completed, found ${summaryResults.length} results`);
    ...
    this.server.tool(
      ...
      async (args: unknown) => {
        console.warn(`[MCP] Tool call received: get-single-web-page-content`);
        console.warn(`[MCP] Raw arguments:`, JSON.stringify(args, null, 2));
        ...
        try {
          ...
          console.warn(`[MCP] Starting single page content extraction for: ${obj.url}`);
          ...
          console.warn(`[MCP] Single page content extraction completed, extracted ${content.length} characters`);
  ...
  private async handleWebSearch(input: WebSearchToolInput): Promise<WebSearchToolOutput> {
    ...
    try {
      ...
      console.warn(`[web-search-mcp] DEBUG: Requesting ${searchLimit} search results to get ${limit} non-PDF content results`);
  ...
  async run(): Promise<void> {
    console.warn('Setting up MCP server...');
    const transport = new StdioServerTransport();
    
    console.warn('Connecting to transport...');
    await this.server.connect(transport);
    console.warn('Web Search MCP Server started');
    console.warn('Server timestamp:', new Date().toISOString());
    console.warn('Waiting for MCP messages...');
  }
}
